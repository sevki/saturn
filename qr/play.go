...
func drawLink(link string) []byte {
	...
	z := 0
	o := 0
	ffsbyts, err := Asset("https://sevki.io/ffbytes.png")
	if err != nil {
		log.Fatal(err)
	}
...
func Qart(w http.ResponseWriter, r *http.Request) {
	dat := drawLink(path.Join("https://ffbyt.es/", Hash(strings.Trim(r.URL.Path, "/"))))
	r.Header.Set("Content-Type", "image/png")
	//Cache-Control: max-age=31556926
	r.Header.Set("Cache-Control", "max-age=31556926")
	w.Write(dat)
	return
}
...
func imageize(pngBuf *bytes.Buffer) []byte {

	i, _, err := image.Decode(pngBuf)
	if err != nil {
		panic(err)
	}

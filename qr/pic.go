...
func makeImage(caption, font string, pt, size, border, scale int, f func(x, y int) uint32) *image.RGBA {
	...
	if csize != 0 {
		dat, err := Asset("static/luxisr.ttf")
		if err != nil {
			panic(err)
		}
...
func makeMask(font string, pt int, vers, mask, scale int) image.Image {
	p, err := coding.NewPlan(coding.Version(vers), coding.L, coding.Mask(mask))
	if err != nil {
		panic(err)
	}
	m := makeImage(maskName[mask], font, pt, len(p.Pixel), 0, scale, func(x, y int) uint32 {
		pix := p.Pixel[y][x]
		switch pix.Role() {
